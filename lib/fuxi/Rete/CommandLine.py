...
def main():
    ...
    op.add_option(
        "--negation", action="store_true", default=False, help="Extract negative rules?"
    )
    ...
    for k, v in list(
        CollapseDictionary(dict([(k, v) for k, v in factGraph.namespaces()])).items()
    ):
        newNsMgr.bind(k, v)
                            ...
                            for otherChild in [o for o in children if o is not child]:
                                if otherChild not in [
                                    ...
                                    warnings.warn(
                                        ...
                                        % (Class(child).qname, Class(otherChild).qname),
                                        UserWarning,
                                        1,
                                    )
    ...
    if options.why:
        ...
        if options.builtinTemplates:
            builtinTemplateGraph = Graph().parse(options.builtinTemplates, format="n3")
        factGraph.templateMap = dict(
            [
                (pred, template)
                for pred, _ignore, template in builtinTemplateGraph.triples(
        ...
        if options.method == "gms":
            ...
            if options.builtinTemplates:
                builtinTemplateGraph = Graph().parse(
                    options.builtinTemplates, format="n3"
                )
                builtinDict = dict(
                    [
                        (pred, template)
    ...
    if options.closure and options.output in RDF_SERIALIZATION_FORMATS:
        ...
        cGraph.namespace_manager = namespace_manager
        print(cGraph.serialize(destination=None, format=options.output, base=None))
    elif options.output and options.output in RDF_SERIALIZATION_FORMATS:
        print(
            network.inferredFacts.serialize(
                destination=None, format=options.output, base=None
            )
        )
