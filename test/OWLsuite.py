        ...
        if REASONING_STRATEGY == "gms":
            for rule in MagicSetTransformation(factGraph, rules, goals, dPreds):
                ...
                if DEBUG:
                    log.debug("\t", rule)
            log.debug(
                "rate of reduction in the size of the program: ",
                (100 - (float(magicRuleNo) / float(progLen)) * 100),
            )

        if REASONING_STRATEGY in ["bfp", "sld"]:  # and not GROUND_QUERY:
            reasoningAlg = (
                TOP_DOWN_METHOD if REASONING_STRATEGY == "sld" else BFP_METHOD
            )
            ...
            for goal in goals:
                ...
                if GROUND_QUERY:
                    ...
                    self.failUnless((goalDict[goal]) in rt, "Failed top-down problem")
            sTime = time.time() - start
            if sTime > 1:
                sTimeStr = "%s seconds" % sTime
            else:
    ...
    def testOwl(self):
        ...
        for manifest in glob("OWL/*/Manifest*.rdf"):
            ...
            for status, premise, conclusion, feature, description in rt:
                ...
                conclusion = manifestGraph.namespace_manager.compute_qname(conclusion)[
                    -1
                ]
                ...
                if status == "APPROVED":
                    ...
                    if REASONING_STRATEGY is None:
                        ...
                        for triple in expectedFacts.parse(
                            ".".join([conclusionFile, "rdf"])
                        ):
                            # closureGraph = ReadOnlyGraphAggregate([self.network.inferredFacts,factGraph])
                            if (
                                triple not in self.network.inferredFacts
...
def runTests(options):
    ...
    if options.profile:
        ...
        s.print_stats(0.1)
        s.print_callers(0.05)
        s.print_callees(0.05)
    else:
        for i in range(options.runs):
...
if __name__ == "__main__":
    ...
    op.add_option("--singleTest", default="", help="The identifier for the test to run")
    op.add_option(
        "--debug",
        "-v",
        action="store_true",
